...
long int largest_of_four(long int one, long int two, long int three, long int four);

int main(){

  // never do this again (M is the # of rows, N is # of columns
  int grid[M][N] = {
  ...
  for(int i = 0 ; i < M ; i++){
	...
	diag2 = grid[i][j]*grid[i-1][j+1]*grid[i-2][j+2]*grid[i-3][j+3];
      ...
      temp = largest_of_four(vert, hor, diag1, diag2);
      if(temp > largest)
	largest = temp;
    }
  }
